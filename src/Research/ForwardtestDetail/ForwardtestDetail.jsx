...
import {primaryButtonStyle, dangerButtonStyle} from '../../constants/styles';
...
class ForwardtestDetail extends Component {
    ...
    constructor(props) {
        ...
        this.handleModeChange = (value) => {
            this.updateState({portfolioMode: value });
        }
        this.handleTransactionModeChange = (value) => {
            this.updateState({transactionMode: value });
        }
        ...
        this.showConfirm = () => {
            this.toggleRestartDialog();
        }
    }

    toggleRestartDialog = () => {
        this.setState({restartDialogOpen: !this.state.restartDialogOpen});
    }
    ...
    renderRestartTestDialog = () => {
        return (
            <DialogComponent 
                open={this.state.restartDialogOpen}
                onClose={this.toggleRestartDialog}
                title="Warning"
                action
                onOk={this.restartTheTest}
                onCancel={this.toggleRestartDialog}
                hideClose
            >
                <span>Are you sure you want to restart the test?</span>
            </DialogComponent>
        );
    }

    renderStopTestDialog = () => {
        return (
            <DialogComponent 
                open={this.state.stopDialogOpen}
                onClose={this.toggleStopDialog}
                title="Warning"
                action
                onOk={() => {
                ...
                onCancel={this.toggleStopDialog}
                hideClose
            >
                <span>Are you sure you want to stop the test?</span>
            </DialogComponent>
        );
    }

    renderDeleteTestDialog = () => {
        return (
            <DialogComponent 
                ...
                hideClose
            >
                <span>Are you sure you want to delete the test?</span>
            </DialogComponent>
        );
    }
    ...
    render() {
            ...
            if (!this.state.loading && !this.state.buttonLoading) {
                ...
                if (this.state.forwardTestData.active) {
                    ...
                    buttons.push(
                        <ButtonBase 
                                key={'restartButton'}
                                style={{...primaryButtonStyle, marginRight: '10px'}}
        ...
        const addMetricData = (currentState) => {
            ...
            for (let i = 0; i < metrics.length; i++) {
                metricsCols.push(
                    <div key={'metrics_cols_' + i} style={metricContainerStyle}>
        ...
        const onTabChanged = tabId => {
            this.setState({selectedTab: Number(tabId)});
        }
        ...
        const getForwardTestTabs = () => {
            if (!this.state.loading) {
                    ...
                    >
                        ...
                        >
                            <RadioGroup 
                                ...
                                onChange={index => {
                                    const value = index === 0 ? 'LatestPortfolio' : 'PortfolioHistory';
                                    this.handleModeChange(value);
                                }}
                ...
                tabs.push(
                    ...
                    >
                        <div style={{
                            ...
                            <RadioGroup 
                                items={['Proposed Orders', 'Transactions']}
                                defaultSelected={this.state.transactionMode === 'ProposedOrders' ? 0 : 1}
                                onChange={index => {
                                    const value = index === 0 ? 'ProposedOrders' : 'Transactions';
                                    this.handleTransactionModeChange(value);
                                }}
                                CustomRadio={CardCustomRadio}
                            />
                ...
                const tabList = [
                    <Tab tabFor='0'>Performance</Tab>,
                    <Tab tabFor='1'>Code</Tab>,
                    <Tab tabFor='2'>Portfolio</Tab>,
                    <Tab tabFor='3'>Transactions</Tab>,
                    <Tab tabFor='4'>Logs</Tab>,
                    <Tab tabFor='5'>Settings</Tab>,
                ]
        ...
        const getTotalDiv = () => {
            if (!this.state.loading) {
                return (
                    <div 
                            lassName="forwardtest-detail-div" 
                            style={{ 
                                padding: '1% 3% 1% 3%',
                                width: '100%',
                                ...
                                <Grid item xs={6}>
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            {
                                                (this.queryParams) 
                                                ? this.queryParams.get('strategyName') 
                                                : ''
                                            }
                                        </h2>
                                        {getLiveTestStatus()}
        ...
        return (
            <AqDesktopLayout loading={this.state.loading}>
                {this.renderRestartTestDialog()}
                {this.renderDeleteTestDialog()}
                {this.renderStopTestDialog()}
                {getTotalDiv()}
            </AqDesktopLayout>
        );
    }
